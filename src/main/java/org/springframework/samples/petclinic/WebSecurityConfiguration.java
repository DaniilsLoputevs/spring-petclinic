...
public class WebSecurityConfiguration {
	...
	private final String apiPetclinicJwkSetUri;

	public WebSecurityConfiguration(UserRepository userRepository,
									ClientRegistrationRepository clientRegistrationRepository,
									@Value("${spring-petclinic.adminui.oauth2.resourceserver.jwt.jks-set-uri}") String adminuiPetclinicJwkSetUri,
									@Value("${spring-petclinic.api.oauth2.resourceserver.jwt.jwk-set-uri}") String apiPetclinicJwkSetUri) {
		this.userRepository = userRepository;
		this.clientRegistrationRepository = clientRegistrationRepository;
		this.adminuiPetclinicJwkSetUri = adminuiPetclinicJwkSetUri;
		this.apiPetclinicJwkSetUri = apiPetclinicJwkSetUri;
	}
...
//                		.forEach(mappedAuthorities::add));
	...
	public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
		...
		http.oauth2ResourceServer(oauth2ResourceServer -> oauth2ResourceServer
			.jwt(jwt -> jwt
				.jwkSetUri(apiPetclinicJwkSetUri)
				.jwtAuthenticationConverter(jwtAuthenticationConverter())));
		http.anonymous(Customizer.withDefaults());
		http.csrf(Customizer.withDefaults());
		return http.build();
