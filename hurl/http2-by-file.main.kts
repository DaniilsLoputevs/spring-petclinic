...
class AsyncClientH2Multiplexing {
    ...
    fun oneByOne(client: MinimalHttpAsyncClient, requestUris: List<String>, allowEtag: Boolean) {
...
//                            println(StatusLine(response).statusCode)
//                            println(response?.headers?.toList())
                        }
...
//        println(fLines.stream().skip(1).limit(50_000).toList().size)
        fLines.stream().skip(1).limit(20_000).toList()
            .shuffled().forEach { ch ->
                println(measureTime {
                    val requestUris = ch.split(",").map { "/rest/owners/with-pets/$it" }
